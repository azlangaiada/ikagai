...
export const seed = async ({
  ...
  payload.logger.info(`— Seeding admin, demo author and user...`)

  // Ensure the template default admin exists so BeforeLogin's displayed
  // credentials actually work on a fresh clone.
  const existingAdmin = await payload.find({
    collection: 'users',
    where: { email: { equals: 'admin@gaiada.com' } },
    limit: 1,
  })
  if (existingAdmin.totalDocs === 0) {
    await payload.create({
      collection: 'users',
      data: { email: 'admin@gaiada.com', password: 'admin', name: 'Admin' },
    })
  }
